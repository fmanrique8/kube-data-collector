...
def compute_momentum_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Stochastic_score'] = df['Stochastic_signal'].map(signal_scores) * Stochastic_weight

    # Sum up the scores to get the momentum score
    df['momentum_score_num'] = df[['ROC_score', 'RSI_score', 'Stochastic_score']].sum(axis=1)

    # Assign momentum score labels based on momentum score num
    df['momentum_score'] = pd.cut(df['momentum_score_num'],
